...
function cal_popup1 (str_datetime) {
	...
	var obj_calwindow = window.open(
		...
		'Calendar', 'width=200,height=185,status=no,resizable=no,top=200,left=200,dependent=yes,alwaysRaised=yes'
	);
...
function cal_prs_date1 (str_date) {
	...
	if (!arr_date[0]) return cal_error ("Formato de data Inválido: '" + str_date + "'.\nO dia do mês no pode ser encontrado.");
	if (!RE_NUM.exec(arr_date[0])) return cal_error ("Dia do mês Inválido: '" + arr_date[0] + "'.\nValores permitidos são inteiros sem sinais.");
	...
	if (arr_date[1] < 1 || arr_date[1] > 12) return cal_error ("Ms invlido: '" + arr_date[1] + "'.\nIntervalo permitido  de 01-12.");
	...
	if (dt_date.getMonth() != (arr_date[1]-1)) return cal_error ("Dia do ms invlido: '" + arr_date[0] + "'.\nIntervalo permitido  de 01-"+dt_numdays.getDate()+".");
